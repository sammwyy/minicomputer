...
pub fn encode_frame<T: Serialize>(opcode: u8, body: &T) -> Result<Vec<u8>, rmp_serde::encode::Error> {
...
pub async fn read_frame<R: tokio::io::AsyncRead + Unpin>(reader: &mut R) -> io::Result<Vec<u8>> {
    ...
    if length == 0 || length > MAX_FRAME { return Err(io::Error::new(io::ErrorKind::InvalidData, "invalid frame length")); }
